...
def dist_train(epoch, encoder, center, encoder_optimizer, datasetloader, device, verbose=1):   
    ...
    for batch_idx, (data, target) in enumerate(datasetloader):
        ...
        loss, dists, target_one_hot = dist_loss(embedding, target, center) # calculate loss
        num_data = len(data)
        del embedding
...
def dist_train_main(encoder, epochs, device, lr, step_size, gamma, traindatasetloader, save_model, patience):
    ...
    for epoch in range(1, epochs + 1):
        train_embedding, train_target = dist_test(encoder, traindatasetloader, device)
        center = compute_center(train_embedding, train_target)
        center, loss, dists, target_one_hot = dist_train(epoch, encoder, center, encoder_optimizer, traindatasetloader, device)
        encoder_scheduler.step()
        torch.save(encoder, './draw_model/model_'+str(epoch)+'.pth')
        np.save('./draw_model/center_'+str(epoch)+'.npy', center.cpu().numpy())
        np.save('./draw_model/loss_'+str(epoch)+'.npy', loss.cpu().detach().numpy())
        np.save('./draw_model/dists_'+str(epoch)+'.npy', dists.cpu().detach().numpy())
        np.save('./draw_model/target_one_hot_'+str(epoch)+'.npy', target_one_hot.cpu().detach().numpy())
